package states.and.state.capitals.statesAndCapitals;
...
//        List<StateInfo> firstFiveStates = null;
        ...
        List<StateInfo> lastFiveStates = states.stream().skip(45).collect(Collectors.toList());
        System.out.println("The Size"+lastFiveStates.stream().count());//50
//        List<StateInfo> lastFiveStates = null;
        ...
        // Assuming State class has a method getYearlyPrecipitation() which returns an integer.
...
//        Map<String, Long> numberOfStatesByTimeZone = null;
        ...
        String allStateNamesSemicolonDelimited = states.stream().map(state->state.getStateName()).collect(Collectors.joining("; "));
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream()
                .map(state->state.getStateBird())
                .distinct()
                .filter(bird -> !bird.equalsIgnoreCase("mockingbird"))
                .collect(Collectors.toList());
